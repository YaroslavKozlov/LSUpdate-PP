...
#
$CTSComponents = @('ctsAuthorization','ctsSynchronization','ctsWeb')
...
else { $scriptUpdateFolderNet = $Global:Update2FolderNet }
...
pause
Write-Host "Распаковка сборки полученной от вендора" -ForegroundColor Yellow
...
Write-Host "Контрольное удаление ненужных первичных файлов в папках сборки  CTS" 
pause
foreach ($CTSComponent in $CTSComponents) {
    $SubCompotentPath = Join-Path $CTSBuild -ChildPath $CTSComponent
...
foreach ($apphost in $HostsRS) {
     ...
     (Join-Path $CTSBuildNet -ChildPath 'ctsWeb'), 
     $CTSBuildNet, $cred
     ...
     foreach ($syncservice in $CTSSyncServices) {
         ...
         } -ArgumentList $syncservice, (Join-Path $CTSBuildNet -ChildPath 'ctsSynchronization'), $cred
     } #foreach (по службам синхронизации)
     Invoke-Command -Session $Apphost {
        Update-DevOpsToolsService -ServiceName $args[0] -SrcInstallDir $args[1] -NetAccount $args[2]
     } -ArgumentList $CTSAuthService, (Join-Path $CTSBuildNet -ChildPath 'ctsAuthorization'), $cred
